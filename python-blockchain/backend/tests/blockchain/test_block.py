...
def test_genesis():
    ...
    for key, value in GENESIS_DATA.items():
       getattr(genesis,key) == value  #get attribute

def test_quickly_mined_block():
    last_block = Block.mine_block(Block.genesis(),'vasu')
...
def test_mined_block_difficulty_limts_at_1():
    last_block = Block(
        time.time_ns(),
        'test_last_hash',
        'test_hash',
...
def test_is_valid_bad_proof_of_word(last_block,block):
    block.hash = 'fff'
...
def test_is_valid_block_bad_block_hash(last_block,block):
    block.hash = '000000000000000bbbabc'
